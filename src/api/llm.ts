...
export async function sendLLMMessage(question: string, chatId: string, model: string, category: string, userId: string) {
    ...
    const res = await api.post(`${BASE_URL}/llm/${chatId}?user_id=${userId}`, {
        question: question,
        model: model,
        category: category
    });
...
export async function getLLMMessages(chatId: string, userId: string) {
    ...
    const res = await api.get(`${BASE_URL}/llm?chat_id=${chatId}&user_id=${userId}`, {
        validateStatus: () => true,
    });
    console.log(`res: ${res}`)
    return res;
...
export async function getLLMChatList(): Promise<ChatHistoryItem[]> {
    ...
    const res = await api.get(`${BASE_URL}/llm/history?user_id=${userId}`);
    return res.data.data;
}

export async function deleteLLMChat(chatId: string, userId: string) {
    const res = await api.delete(`${BASE_URL}/llm/reset/${chatId}?user_id=${userId}`);
    return res.status;
}

export async function deleteLLMChatAll(userId: string) {
    const res = await api.delete(`${BASE_URL}/llm/reset_all?user_id=${userId}`);
    return res;
}